...
char* deleteLastChar(char* s)
{
    ...
    free(s); // Free the memory of the input string
...
int main()
{
    ...
    strncpy(tab2, tab1, l); // Copy tab1 to tab2
...
/*
- Request:
I'm trying to make a program which is deleting the last char from string over and over again until there is no more chars in it. It has to be done on dynamic arrays by shortening new arrray and rewrite previous array into it.
...
- Error:
Program is compiling without any errors, although it doesn't work as it suppouse to do.
After putting an input it is outputting "e" multiple times.
