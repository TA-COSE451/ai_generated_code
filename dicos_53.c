...
int factorial(int num, void (*mock_function)(void))
{
    int fact = 1;
    printf("__%s__()\n", __func__);
    while (num > 1)
    ...
    // Call the provided function pointer (either dummy or mocked_dummy)
    mock_function();
...
int main(int argc, char *argv[])
{
    // By default, use the dummy function
    void (*mock_function)(void) = dummy;

    if (argc > 1 && atoi(argv[1]) == 1) {
        // Use mocked_dummy if the command line argument is 1
        mock_function = mocked_dummy;
    }

    printf("factorial of 5 is = %d\n", factorial(5, mock_function));
    return 0;
}
